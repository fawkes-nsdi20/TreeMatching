...
# Runs as a command from root directory
APTED_REL_PATH: str = 'HTML-APTED/build/libs/apted.jar'

# Reads the file relative to this package (treematching)
PATCHER_REL_PATH: str = '../JSPatcher/built/patcher.js'
...
def get_apted_edits(first: Tree, second: Tree, out_path: str) -> EditSequence:
    apted_out = run_command(f'java -jar {APTED_REL_PATH} -f {out_path}_1.tree {out_path}_2.tree -m')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        sys.exit('Usage: python3.7 treematching/run_apted.py first_html second_html out_path [html|json]')
    ...
    with open(first_path, 'r') as first_file_handle, \
        ...
        if goal == 'html':
            ...
            patcher_path: str = os.path.join(current_dir, PATCHER_REL_PATH)
